...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.power_level > 1 and pygame.time.get_ticks() - self.power_timer > 5000:
            self.power_level = 1  # Reset to 1 after 5 seconds
        ...
        # Keep ship fully visible on screen
        if self.rect.left < 0:
            self.rect.left = 0
 ...
 lesquels  def powerup(self):
        self.power_level = min(2, self.power_level + 1)  # Cap at 2
        self.power_timer = pygame.time.get_ticks()
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):  # Fixed typo from update噴
...
# Text effect class for power-up visuals
...
def main_game():
    ...
    while running:
        ...
        hits = pygame.sprite.groupcollide(enemies, bullets, True, True)  # Destroy enemies
        for hit in hits:
            score += 100
        ...
        for hit in hits:
            ...
            if player.shield <= 0:
                ...
                player.shield = 100
                player.hide()
                if player.lives <= 0:  # Changed to <= 0 for clarity
                    game_over = True
        ...
        for hit in hits:
            if hit.type == 'shield':
                ...
                if player.shield > 100:
                    player.shield = 100
                text_effect = TextEffect("+20 Shield!", player.rect.centerx, player.rect.centery)
                all_sprites.add(text_effect)
            if hit.type == 'power':
                player.powerup()
                text_effect = TextEffect("Power Up!", player.rect.centerx, player.rect.centery)
                all_sprites.add(text_effect)
